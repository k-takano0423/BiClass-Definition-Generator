...
def make_prompt_turning_prompt(definition_statement, false_negative_text_list, false_positive_text_list, MAX_SAMPLING_FALSE_NEGA_DATA_NUM, MAX_SAMPLING_FALSE_POSI_DATA_NUM):
    prompt = definition_statement + PROMPT_DICT['prompt_turning_prompt']
    if len(false_negative_text_list) > 0:
        print('False Negative Text List')
        for [text, importance] in false_negative_text_list:
            print('importance -> ', importance)
            print(text)
    if len(false_negative_text_list) > 0:
        inputs_text_list = []
        for no in range(MAX_SAMPLING_FALSE_NEGA_DATA_NUM):
            text = input(f'Please Enter False Negative Text{no+1}: ')
            if len(text) == 0:
                break
            else:
                inputs_text_list.append(text)
        if len(inputs_text_list) > 0:
            prompt = prompt + PROMPT_DICT['false_negative_prompt']
            for text in inputs_text_list:
                prompt = prompt + '\n' + text

    if len(false_positive_text_list) > 0:
        print('False Positive Text List')
        for [text, importance] in false_positive_text_list:
            print('importance -> ', importance)
            print(text)
    if len(false_positive_text_list) > 0:
        inputs_text_list = []
        for no in range(MAX_SAMPLING_FALSE_POSI_DATA_NUM):
            text = input(f'Please Enter False Positive Text{no+1}: ')
            if len(text) == 0:
                break
            else:
                inputs_text_list.append(text)
...
def turning_step_func(df, definition_statement, model_name="gpt-4o-mini-2024-07-18", MAX_SAMPLING_FALSE_NEGA_DATA_NUM=2, MAX_SAMPLING_FALSE_POSI_DATA_NUM=2):
    ...
    temp_df = true_df.sample(len(true_df), weights='importance', random_state=0)
    for text, importance in tqdm(zip(temp_df['text'], temp_df['importance'])):
        prompt = make_classify_prompt(text, definition_statement)
        pred_label = generate_text(prompt, model_name)
        if 'False' in pred_label:
            false_negative_text_list.append([text, importance])
    false_df = df[df['label']==0].copy()
    false_positive_text_list = []
    temp_df = false_df.sample(len(false_df), weights='importance', random_state=0)
    for text, importance in tqdm(zip(temp_df['text'], temp_df['importance'])):
        prompt = make_classify_prompt(text, definition_statement)
        pred_label = generate_text(prompt, model_name)
        if 'True' in pred_label:
            false_positive_text_list.append([text, importance])
    if len(false_negative_text_list) + len(false_positive_text_list) == 0:
        return '', definition_statement, True

    prompt = make_prompt_turning_prompt(definition_statement, false_negative_text_list, false_positive_text_list, MAX_SAMPLING_FALSE_NEGA_DATA_NUM, MAX_SAMPLING_FALSE_POSI_DATA_NUM)
    print(prompt)
    print('-------------------------------')
    generated_output = generate_text(prompt, model_name)
    print('Generated Definition: ')
    print(generated_output)
    turning_definition_statement = input('Please Enter Definition: ')
...
def main():
    ...
    for k in range(1, CONFIG['N_SPLIT']):
        ...
        if definition_statement is None:
            ...
            print(generated_output)
            # 入力を受け取る
            definition_statement = input('Please Enter Definition: ')
    ...
    pd.DataFrame(definition_statement_list, columns=['definition_statement']).to_excel(f"{CONFIG['OUTPUT_PATH']}/{CONFIG['EXPERIMENT_NAME']}_HITL_definition_statements.xlsx", index=False)
    ...
    valid_df.to_excel(f"{CONFIG['OUTPUT_PATH']}/{CONFIG['EXPERIMENT_NAME']}_HITL_valid.xlsx", index=False)
    test_df.to_excel(f"{CONFIG['OUTPUT_PATH']}/{CONFIG['EXPERIMENT_NAME']}_HITL_test.xlsx", index=False)
